...
const { ZERO_ADDRESS, METADATA_URI, PREDICTION } = constants;
    ...
    function () {
        context(`${contract}`, function () {
            ...
            beforeEach(async function () {
                ...
                // append address
                rootAddress = this.erc721a.address;
            });
            ...
            context("with minted tokens", async function () {
                ...
                describe("ERC721Metadata support", async function () {
                    it("responds with the right name", async function () {
                        expect(await this.erc721a.name()).to.eq("PollsMarket");
                    });

                    it("responds with the right symbol", async function () {
                        expect(await this.erc721a.symbol()).to.eq("PMT");
                    });

                    describe("tokenURI", async function () {
                        it("sends a DEFAULT metadata _uri", async function () {
                            const uri = await this.erc721a["tokenURI(uint256)"](1);
                            expect(uri.toLowerCase()).to.eq(`${METADATA_URI}${this.erc721a.address}/1`.toLowerCase());
                        });
                    ...
                    describe("contractURI", async function () {
                        it("sends a contract URI with root URI", async function () {
                            const uri = await this.erc721a.contractURI();
                            expect(uri).to.eq(`${METADATA_URI}${this.erc721a.address}`.toLowerCase());
                        });
                    });
                });
                ...
                describe("predictionOf", async function () {
                ...
                describe("approve", async function () {
                    ...
                    it("rejects an invalid token owner", async function () {
                        await expect(this.erc721a.connect(this.addr1).approve(this.addr2.address, this.tokenId2)).to.be.revertedWith("ApprovalToCurrentOwner");
                    });

                    it("rejects an unapproved caller", async function () {
                        await expect(this.erc721a.approve(this.addr2.address, this.tokenId)).to.be.revertedWith("ApprovalCallerNotOwnerNorApproved");
                    });
                ...
                describe("setApprovalForAll", async function () {
                    it("sets approval for all properly", async function () {
                        ...
                        await expect(approvalTx).to.emit(this.erc721a, "ApprovalForAll").withArgs(this.owner.address, this.addr1.address, true);
                    ...
                    it("sets rejects approvals for non msg senders", async function () {
                        await expect(this.erc721a.connect(this.addr1).setApprovalForAll(this.addr1.address, true)).to.be.revertedWith("ApproveToCaller");
                    });
                });
                ...
                context("test transfer functionality", function () {
                    ...
                    const testUnsuccessfulTransfer = function (transferFn: string) {
                        ...
                        it("rejects unapproved transfer", async function () {
                            await expect(this.erc721a.connect(this.addr1)[transferFn](this.addr2.address, this.addr1.address, this.tokenId)).to.be.revertedWith(
                        ...
                        it("rejects transfer from incorrect owner", async function () {
                            ...
                            await expect(this.erc721a.connect(this.addr1)[transferFn](this.addr3.address, this.addr1.address, this.tokenId)).to.be.revertedWith(
                        ...
                        it("rejects transfer to zero address", async function () {
                            await this.erc721a.connect(this.addr2).setApprovalForAll(this.addr1.address, true);
                            await expect(this.erc721a.connect(this.addr1)[transferFn](this.addr2.address, ZERO_ADDRESS, this.tokenId)).to.be.revertedWith(
            ...
            context("mint", async function () {
                ...
                describe("safeMint", function () {
                    it("successfully mints a single token", async function () {
                        ...
                        await expect(mintTx).to.emit(this.erc721a, "Transfer").withArgs(ZERO_ADDRESS, this.receiver.address, this.startTokenId);
                        await expect(mintTx)
                            .to.emit(this.receiver, "Received")
                    ...
                    it("successfully mints multiple tokens", async function () {
                        ...
                        for (let tokenId = this.startTokenId; tokenId < 5 + this.startTokenId; tokenId++) {
                            ...
                            await expect(mintTx).to.emit(this.receiver, "Received").withArgs(this.owner.address, ZERO_ADDRESS, tokenId, "0x", GAS_MAGIC_VALUE);
                    ...
                    it("rejects mints to the zero address", async function () {
                        await expect(this.erc721a["safeMint(address,uint256)"](ZERO_ADDRESS, 1)).to.be.revertedWith("MintToZeroAddress");
                    });

                    it("requires quantity to be greater than 0", async function () {
                        await expect(this.erc721a["safeMint(address,uint256)"](this.owner.address, 0)).to.be.revertedWith("MintZeroQuantity");
                    });
...
describe("ERC721A", createTestSuite({ contract: "ERC721AMock", constructorArgs: ["PollsMarket", "PMT"] }));

describe("ERC721A override _startTokenId()", createTestSuite({ contract: "ERC721AStartTokenIdMock", constructorArgs: ["PollsMarket", "PMT", 1] }));
