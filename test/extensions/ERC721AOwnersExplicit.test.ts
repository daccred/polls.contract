    ...
    function () {
        context(`${contract}`, function () {
            beforeEach(async function () {
                ...
                this.startTokenId = this.erc721aOwnersExplicit.startTokenId ? (await this.erc721aOwnersExplicit.startTokenId()).toNumber() : 0;
            });
            ...
            context("with minted tokens", async function () {
                ...
                describe("setOwnersExplicit", async function () {
                    ...
                    it("handles single increment properly", async function () {
                        ...
                        expect(await this.erc721aOwnersExplicit.nextOwnerToExplicitlySet()).to.equal((1 + this.startTokenId).toString());
                    });
                    ...
                    it("properly sets the ownership of index 2", async function () {
                        ...
                        expect(await this.erc721aOwnersExplicit.nextOwnerToExplicitlySet()).to.equal((3 + this.startTokenId).toString());
                    });
                    ...
                    it("sets all ownerships in multiple calls", async function () {
                        ...
                        expect(await this.erc721aOwnersExplicit.nextOwnerToExplicitlySet()).to.equal((2 + this.startTokenId).toString());
                        await this.erc721aOwnersExplicit.setOwnersExplicit(1);
                        expect(await this.erc721aOwnersExplicit.nextOwnerToExplicitlySet()).to.equal((3 + this.startTokenId).toString());
                        await this.erc721aOwnersExplicit.setOwnersExplicit(3);
                        for (let tokenId = this.startTokenId; tokenId < 6 + this.startTokenId; tokenId++) {
                    ...
                    it("rejects after all ownerships have been set", async function () {
                        await this.erc721aOwnersExplicit.setOwnersExplicit(6);
                        await expect(this.erc721aOwnersExplicit.setOwnersExplicit(1)).to.be.revertedWith("AllOwnershipsHaveBeenSet");
                    });
                });
            });
        });
