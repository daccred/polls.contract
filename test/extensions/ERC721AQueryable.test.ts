...
const createTestSuite = ({ contract, constructorArgs, setOwnersExplicit = false }: any) =>
    function () {
        let offseted: any;
        ...
        context(`${contract}`, function () {
            beforeEach(async function () {
                ...
                this.startTokenId = this.erc721aQueryable.startTokenId ? (await this.erc721aQueryable.startTokenId()).toNumber() : 0;

                offseted = (...arr: any[]) => arr.map((num) => BigNumber.from(this.startTokenId + num));
            });

            const expectExplicitOwnershipBurned = function (explicitOwnership: { burned: any; addr: any; startTimestamp: any }, address: any) {
            ...
            const expectExplicitOwnershipNotExists = function (explicitOwnership: { burned: any; addr: any; startTimestamp: any }) {
            ...
            const expectExplicitOwnershipExists = function (explicitOwnership: { burned: any; addr: any; startTimestamp: any }, address: any) {
                expect(explicitOwnership.burned).to.eql(false);
                expect(explicitOwnership.addr).to.eql(address);
                expect(explicitOwnership.startTimestamp).to.not.eql(BigNumber.from(0));
            };
            ...
            context("with minted tokens", async function () {
                ...
                describe("tokensOfOwnerIn", async function () {
                    const expectCorrect = async function (this: any, addr: any, start: any, stop: any) {
                        if (BigNumber.from(start).gte(BigNumber.from(stop))) {
                            await expect(this.erc721aQueries.tokensOfOwnerIn(addr, start, stop)).to.be.revertedWith("InvalidQueryRange");
                        } else {
                            const expectedTokens = (await this.erc721aQueryable.tokensOfOwner(addr)).filter(
                                (x: BigNumberish) => BigNumber.from(start).lte(x) && BigNumber.from(stop).gt(x)
                            );
                    ...
                    const subTests = function (
                        description: string,
                        beforeEachFunction: { (): Promise<void>; (): Promise<void>; (): Promise<void>; call?: any }
                    ) {
                    ...
                    subTests("after a token tranfer", async function (this: any) {
                        await this.erc721aQueryable.transferFrom(this.owner.address, this.addr4.address, offseted(7)[0]);
                    });

                    subTests("after a token burn", async function (this: any) {
                ...
                describe("explicitOwnershipsOf", async function () {
                    ...
                    it("out of bounds", async function () {
                        const tokenIds = [].concat([this.currentIndex] as any, this.addr3.expected.tokens);
                        const explicitOwnerships = await this.erc721aQueryable.explicitOwnershipsOf(tokenIds);
...
describe(
    ...
    createTestSuite({
        ...
        constructorArgs: ["PollMarket", "PMT"],
    })
);
...
describe(
    ...
    createTestSuite({
        ...
        constructorArgs: ["PollMarket", "PMT", 1],
    })
);
