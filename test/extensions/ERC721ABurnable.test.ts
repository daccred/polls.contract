    ...
    function () {
        context(`${contract}`, function () {
            beforeEach(async function () {
                ...
                this.startTokenId = this.erc721aBurnable.startTokenId ? (await this.erc721aBurnable.startTokenId()).toNumber() : 0;
            });
            ...
            it("cannot transfer a burned token", async function () {
                const query = this.erc721aBurnable.connect(this.addr1).transferFrom(this.addr1.address, this.addr2.address, this.burnedTokenId);
                await expect(query).to.be.revertedWith("OwnerQueryForNonexistentToken");
            });
            ...
            describe("ownerships correctly set", async function () {
                it("with token before previously burnt token transferred and burned", async function () {
                    ...
                    await this.erc721aBurnable.connect(this.addr1).transferFrom(this.addr1.address, this.addr2.address, tokenIdToBurn);
                ...
                it("with last token burned", async function () {
                    await expect(this.erc721aBurnable.ownerOf(this.numTestTokens + this.startTokenId)).to.be.revertedWith("OwnerQueryForNonexistentToken");
                    await this.erc721aBurnable.connect(this.addr1).burn(this.numTestTokens - 1 + this.startTokenId);
                    await expect(this.erc721aBurnable.ownerOf(this.numTestTokens - 1 + this.startTokenId)).to.be.revertedWith("OwnerQueryForNonexistentToken");
                });
            });
        });
    };
...
describe("ERC721ABurnable override _startTokenId()", createTestSuite({ contract: "ERC721ABurnableStartTokenIdMock", constructorArgs: ["Daccred", "DCD", 1] }));
